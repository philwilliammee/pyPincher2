...
class pose(list):
    ...
    def __init__(self, line, length):
        ...
        #I have changed this to float used to be integers
        try:
            for servo in range(length): 
                if line.find(",") > 0:
                    self.append(float(line[0:line.index(",")]))       
                else:
                    self.append(float(line[0:]))
...
#@todo the scope of this should be increased to include different models resolutions ect
class project:
    def __init__(self):
        #pincher model values
        self.name = "pincher2"
        self.count = 5 # Max number of servos
        self.resolution = [1024 for i in range(self.count)]
        self.safety_limits = [[1023, 0], [975, 105],[1016, 51], [837,171], [960,10]]
        self.links = [0.0, 105.0, 105.0, 113.0] #98.0])# actual measurements of segment length in mm
        self.poses = dict()#a dictionary of poses
        self.sequences = dict()
        self.nuke = "IK_engine"    
        self.save = False
    ...
    def load(self, filename):
        ...
        print "loaded sequences", self.sequences
        self.save = False
...
def extract(li):
    ...
    out = list()
    #print ("extracting valus from this list" + str(li))
    for i in li:
        out = out + [i%256, i>>8]
